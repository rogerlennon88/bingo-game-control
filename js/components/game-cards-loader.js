...
class GameCardsLoader {
  ...
  confirmAndLoad(file) {
    this.countRecords(file)
      .then((recordCount) => {
        if (confirm(`Se van a importar ${recordCount} registros. ¿Continuar?`)) {
  ...
  saveCSVData(csvData) {
    fetch("/save-csv", {
      ...
      .catch((error) => {
        ...
        systemMessage.show(error.message || "Error al enviar la solicitud.", "danger")
      })
  }
  ...
  showError(error) {
    this.hideLoading()
    this.showMessage(error || "Error al cargar el archivo CSV. Intente de nuevo.", "error")
  }
